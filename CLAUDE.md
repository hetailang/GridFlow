...
- `ControlPanel.jsx` — Settings sidebar. Phase 1: layout/styling/export. Phase 2: return button, selected element properties (x/y/w/h/rotation/cornerRadius), crop controls (cropZoom/cropOffsetX/cropOffsetY) + "自适应图片大小" button (sets height = width × nH/nW and resets crop params so image fits without clipping), layer order buttons, export.
